...
function LostFoundPersonal() {
  ...
  useEffect(() => {
    fetch(
      "http://localhost:3000/api/lost-found/personal",
      {
        method: 'POST',
        headers : {
        ...
        body: JSON.stringify({"author_email":cookies.email})
      })
      .then((res) => res.json())
      .then((json) => {
        setPosts(json);
  ...
  return (
    <div className="Personal" style={{marginTop: "4rem"}}>
      {isFetching ? <></>
       : posts.map( post =>
...
export default LostFoundPersonal;
